...
public class MongoTask implements Callable<Boolean>, Runnable, Serializable {
    ...
    public MongoTask(LinkedBlockingQueue<DBObject> queue, String url, String dbName, String[] collectionNames, String[] filterNameSpaces, String[] ns) {
        this.queue = queue;
        this.collectionNames = collectionNames;
        this.filterNameSpaces = Sets.newHashSet(filterNameSpaces);
        this.nameSpaces = Sets.newHashSet(ns);
        ...
        while (running.get()) {
            try {
                ...
                if (this.cursor.hasNext()) {
                    ...
                    if(!nameSpaces.isEmpty() && !nameSpaces.contains(object.get("ns").toString()))
                    {
                        continue;
                    }
