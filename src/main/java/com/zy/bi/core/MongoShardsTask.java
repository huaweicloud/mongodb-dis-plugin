...
public class MongoShardsTask implements Serializable  {
    ...
    public MongoShardsTask(LinkedBlockingQueue<DBObject> queue, String url, String dbName, String[] collectionNames, String[] filterNameSpaces, String[] ns) {
        this.queue = queue;
        this.dbName = dbName;
    ...
    private void initializeMongoShardTasks(String url) {
        ...
        List<MongoShardInfo> shardInfos = MongoUtil.getMongoShards(mongos,MongoUtil.getUserAndPassword(url));
        mongoTasks = Lists.newArrayList();
        for (MongoShardInfo mongoShardInfo : shardInfos) {
            MongoTask mongoTask = new MongoTask(this.queue, mongoShardInfo.getHost(), this.dbName, this.collectionNames, this.filterNameSpaces,this.nameSpace);
